...
- E2E:
  ...
  - [x] **실제 카카오페이 테스트 결제 성공** (PC 웹 → QR → 폰 승인 방식)
        서버 로그: 주문 20260705-0020 결제완료 + 129P 적립 확인 ✅ → 결제 연동 완료
  - 참고: 태블릿에서는 카카오페이 "주 사용 기기" 제한으로 최종 승인 불가 (PC 웹 QR 방식으로 우회)
  - 수단별 채널 키 매핑 추가(PR #41): 콘솔에 카드 채널 추가 시 PORTONE_CHANNEL_KEY_CARD로 연결
