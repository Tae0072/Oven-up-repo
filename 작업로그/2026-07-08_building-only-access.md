...
## 3차: 위치 설정 바로가기 (2026-07-08 17:00 KST, PR #61)
권한 안내만 뜨고 끝나던 것을 개선 — 차단 사유를 구분해 다이얼로그로 안내:
- 권한 거부 → "위치 권한이 필요해요" + **[설정 열기]** 버튼 (앱 권한 설정 화면 직행)
- 기기 GPS 꺼짐 → "위치(GPS)가 꺼져 있어요" + **[위치 설정 열기]** 버튼
- 웹은 브라우저 보안 정책상 설정을 코드로 못 열어 자물쇠 아이콘 → 위치 허용 방법을 안내
- 서버 변경 없음. 웹·APK 재배포 완료 (APK: 오븐업\ovenup-release-building-only.apk 갱신)

## 실기기 확인 (2026-07-08)
- 휴대폰(SM-F966N)에 새 APK 설치(adb, ⚠️ 한글 경로의 APK는 adb install 실패 → D:\dev로 복사 후 설치)
- 작업자 확인: 결제하기 → 위치 권한 다이얼로그 → [설정 열기] 버튼 정상 동작 ✅

## 남은 확인
- 태블릿에도 같은 APK 설치 (연결 시 adb install -r, 또는 파일 전송 설치)
- 실사용 재확인: 건물 밖에서 주문 시도 → 차단되는지
