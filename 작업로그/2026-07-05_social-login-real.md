...
- [x] **웹 로그인 E2E 성공** (카카오·네이버 모두, 2026-07-05 밤)
  - 네이버: 콘솔에 "PC 웹" 환경 추가 필요했음 (서비스/콜백 URL: http://localhost:8091)
  - 카카오(새 콘솔 기준) 필요했던 설정 3가지:
    1) 플랫폼 키 → **REST API 키**에 카카오 로그인 리다이렉트 URI `http://localhost:8091` 등록
       (JavaScript 키에 등록해도 REST 키 요청에는 적용 안 됨 → KOE006)
    2) 클라이언트 시크릿(카카오 로그인용)이 발급돼 있으면 서버가 함께 보내야 함 → KOE010
       → `KAKAO_CLIENT_SECRET` 환경변수 지원 추가(PR #42), secrets 파일에 코드 추가
    3) 서버 로그에 카카오/네이버 거절 사유(body)를 남기는 진단 로그 추가 → 원인 즉시 특정 가능
